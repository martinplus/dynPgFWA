...
public class testfunc {
	...
	void test_func(double[] x, double[] f, int nx, int mx,int func_num)throws Exception{
		...
		if (ini_flag==0) /* initiailization*/
		{
			...
			/*Load Matrix M*****************************************************/
			File fpt = new File("E:\\改进FWA实验\\AFWAOr\\input_data/M_"+func_num+"_D"+nx+".txt");//* Load M data *
			...
			if(cf_num>1){
				...
				fpt = new File("E:\\改进FWA实验\\AFWAOr\\input_data/bias_"+func_num+".txt");//* Load bias data *
				input = new Scanner(fpt);
				if (!fpt.exists())
				{
				    System.out.println("\n Error: Cannot open input file for reading ");
			...
			fpt=new File("E:\\改进FWA实验\\AFWAOr\\input_data/shift_data_"+func_num+".txt");
			input = new Scanner(fpt);
			if (!fpt.exists())
			{
				System.out.println("\n Error: Cannot open input file for reading ");
			...
			if(bShuffle[func_num]==1){
				fpt = new File("E:\\改进FWA实验\\AFWAOr\\input_data/shuffle_data_"+func_num+"_D"+nx+".txt");
				input = new Scanner(fpt);
				if (!fpt.exists())
				{
				    System.out.println("\n Error: Cannot open input file for reading ");
